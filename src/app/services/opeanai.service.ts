...
import { Injectable } from '@angular/core';
import { HttpClient, HttpHeaders } from '@angular/common/http';
import { Observable, throwError, catchError, map, forkJoin, delay } from 'rxjs';
import { BoardMembersBreakdown } from '../models/boardcomposition.model';
import { EthicalBehaviorData } from '../models/ethicalbehavior.model';
import { SocialEthicalData } from '../models/social-ethical.model';
import { EnvironmentalEthicalData } from '../models/environmental-ethical.model';
...
export class OpenAIService {
    ...
    generateKeyHighlightsPage(
        ...
        Design specifications:
        - Corporate-style infographic format
        - Use iconography and mini-charts
        - Modern, clean layout with sections for each ESG Pillar
        - Executive-friendly color palette
    ...
    generateBoardDiversityPage(
        ...
        const prompt = `Generate a comprehensive infographic for Board Diversity under the Governance Pillar in the ${companyName} ESG Report ${year}.
        The data includes:
        - Total Board Members: ${boardData.totalboardmembers}
        - Gender Breakdown: ${genderData}
        - Race Breakdown: ${raceData}
        - Age Group Breakdown: ${ageData}
        Design specs:
        - Bar chart (gender), pie chart (race), pie chart (age)
    ...
    generateEthicalBehaviorPage(
        ...
        const trainingData = `${ethicsData.employeeTrainingPercentage}% of employees (${ethicsData.employeesCoveredByTraining}) receive ethics training ${ethicsData.trainingFrequency.toLowerCase()}`;
        const whistleblowerData = `Whistleblower mechanism: ${ethicsData.whistleblowerMechanismExists ? 'Yes' : 'No'}, Anonymous: ${ethicsData.whistleblowerAnonymousReporting ? 'Yes' : 'No'}, Protection policy: ${ethicsData.whistleblowerProtectionPolicy ? 'Yes' : 'No'}`;
        const incidentData = `${ethicsData.numberOfReportedIncidents} reported, ${ethicsData.numberOfInvestigatedIncidents} investigated, ${ethicsData.numberOfResolvedIncidents} resolved, Avg resolution time: ${ethicsData.averageResolutionTimeInDays} days`;

        const prompt = `Generate an infographic for Ethical Behavior under the Governance Pillar in the ${companyName} ESG Report ${year}.
        Data:
        - Code of Ethics: ${ethicsData.codeOfEthicsExists ? 'Yes' : 'No'}, Public: ${ethicsData.codeOfEthicsPubliclyAvailable ? 'Yes' : 'No'}, Updated: ${ethicsData.codeOfEthicsLastUpdated}
        - Ethics Training: ${trainingData}
        - Whistleblower Protection: ${whistleblowerData}
        - Incidents: ${incidentData}
        - Governance: Officer: ${ethicsData.ethicsOfficerExists ? 'Yes' : 'No'}, Committee: ${ethicsData.boardEthicsCommitteeExists ? 'Yes' : 'No'}, Audit Frequency: ${ethicsData.ethicsAuditFrequency}
        - Supplier Code of Conduct: ${ethicsData.supplierCodeOfConductExists ? 'Yes' : 'No'}
        Format: 1024x1024, executive-friendly dashboard`;
    ...
    generateSocialImpactPage(
        ...
        const laborData = `Labor Rights Policy: ${socialData.laborRightsPolicy ? 'Yes' : 'No'}, Public: ${socialData.laborRightsPolicyPublic ? 'Yes' : 'No'}, Due Diligence: ${socialData.humanRightsDueDiligence ? 'Yes' : 'No'}, Frequency: ${socialData.humanRightsDueDiligenceFrequency}`;
        const employeeData = `${socialData.collectiveBargainingPercentage}% covered, ${socialData.employeeGrievances} grievances filed, ${socialData.resolvedEmployeeGrievances} resolved, Formal mechanism: ${socialData.grievanceMechanism ? 'Yes' : 'No'}`;
        const communityData = `Investment: ${socialData.communityInvestment} ZAR (${socialData.communityInvestmentPercentage}%), Initiatives: ${socialData.communityInitiatives}, Volunteer hours: ${socialData.volunteerHours}`;
        ...
        - Employee Relations: ${employeeData}
        - Community Engagement: ${communityData}
        Visual: dashboard layout, 1024x1024, blue/purple/grey palette`;
    ...
    generateEnvironmentalPage(
        ...
        const policyData = `Policy: ${environmentalData.environmentalPolicy ? 'Yes' : 'No'}, Public: ${environmentalData.environmentalPolicyPublic ? 'Yes' : 'No'}, Climate Strategy: ${environmentalData.climateChangeStrategy ? 'Yes' : 'No'}`;
        const emissionsData = `Carbon neutral by ${environmentalData.carbonNeutralByYear}, Reduction target: ${environmentalData.emissionsReductionTarget}%, Offset: ${environmentalData.carbonOffsetProgram ? 'Yes' : 'No'}, Renewable: ${environmentalData.renewableEnergyPercentage}%`;
        const resourceData = `Waste reduction: ${environmentalData.wasteReductionTarget}%, Circular Economy: ${environmentalData.circularEconomyInitiatives ? 'Yes' : 'No'}, Water conservation: ${environmentalData.waterConservationProgram ? 'Yes' : 'No'}`;
        ...
        - Resource Management: ${resourceData}
        Visual format: 1024x1024, greens/blues/greys palette`;
    ...
    generateESGReport(
        ...
        return forkJoin({
            ...
            map(images => this.compilePdfReport(companyName, year, images)),
            catchError(error => {
                console.error('Error generating report:', error);
                return throwError(() => new Error('Failed to generate ESG report. Please try again.'));
            })
        ...
        <html>
            <head>
                ...
                <style>
                    body { font-family: Arial, sans-serif; margin: 0; padding: 0; }
                    .page { page-break-after: always; height: 100vh; }
                    .cover img, .content img { max-width: 100%; max-height: 90vh; }
                    h1 { color: #333; text-align: center; }
                    .footer { text-align: center; margin-top: 20px; font-size: 12px; }
                </style>
            </head>
            <body>
                <div class="page cover"><img src="${images.coverPage}" /></div>
                <div class="page"><h1>Key Highlights</h1><div class="content"><img src="${images.keyHighlights}" /></div><div class="footer">${companyName} ESG Report ${year}</div></div>
                <div class="page"><h1>Board Diversity</h1><div class="content"><img src="${images.boardDiversity}" /></div><div class="footer">${companyName} ESG Report ${year}</div></div>
                <div class="page"><h1>Ethical Behavior</h1><div class="content"><img src="${images.ethicalBehavior}" /></div><div class="footer">${companyName} ESG Report ${year}</div></div>
                <div class="page"><h1>Social Impact</h1><div class="content"><img src="${images.socialImpact}" /></div><div class="footer">${companyName} ESG Report ${year}</div></div>
                <div class="page"><h1>Environmental Sustainability</h1><div class="content"><img src="${images.environmental}" /></div><div class="footer">${companyName} ESG Report ${year}</div></div>
            </body>
        </html>`;
